...
## Acceptance Criteria
- [ ] Push result is checked; failure is reported on stderr with the actual git error, never a success receipt.
- [ ] On rejected push: fetch + rebase (fast-forward-safe), retry once; if still failing, surface loudly and keep the local commit.
- [ ] sync-data gets its first test file (success, push-rejected, divergence-rebase paths) wired into `npm test`.
- [ ] Receipt is emitted ONLY when something was actually committed/pushed (no-op turns stay silent).
