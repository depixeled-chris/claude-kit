...
- 2026-06-06 (refinement, maintainer correction): surfacing ALL standing decisions every session is unsustainable + token-wasteful. Now SCOPE-FILTERED: a standing decision declares `scope:` (free token) and/or `paths:` (comma globs); orient surfaces only those whose scope matches the active signals (changed files + recent commit subjects) or whose paths-glob matches a changed file; the rest collapse to a one-line "+N out of scope (scopes: …)" pointer. Generic — no project taxonomy in the kit; the decision declares its own relevance. Verified: touching rust/ surfaces the 4 world-gen decisions; touching render surfaces none + pointer; clean tree → pointer only.
