...
3. Dedupe against existing tickets — `node <kit>/scripts/q.mjs similar "<proposed title/labels>"`
   surfaces likely-duplicate tickets (FTS, suggest-only). On a real duplicate either skip it, link
   it (`links:`), or — if it RETIRES an older one — set `supersedes: <old-id>` on the new ticket and
   `superseded_by: <new-id>` (or `status: superseded`) on the old, which drops the old from the
   board + drain (KIT-T024). For regressions, link the likely causing commit.
