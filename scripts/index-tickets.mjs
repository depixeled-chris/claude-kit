...
export async function regenerateIndexes(root) {
  ...
  const row = (t) => `| ${t.id} | ${t.type} | ${t.status} | ${t.priority} | ${t.title}${akaTag(t)}${parentTag(t)} |`;
  const supRow = (t) => `| ${t.id} | ${t.status} | ${t.title}${akaTag(t)}${parentTag(t)} | ${t.supersededBy || '—'} |`;
  ...
  // Build reverse index: parent id → child items (one pass, graceful on dangling parents).
  ...
  for (const t of tickets) {
    ...
    // dangling parent (target missing): skip silently — no crash.
  }
  const childrenRollup = (t) => {
    const kids = childrenByParent.get(t.id);
    return kids && kids.length ? `\n|  | | | | ↳ children: ${kids.join(', ')} |` : '';
  };
  const rowWithRollup = (t) => row(t) + childrenRollup(t);
